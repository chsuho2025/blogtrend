...
오늘 급상승 단어를 보고 내일 블로그에서 다양한 트렌드 콘텐츠가 많이 나올 것 같은 검색 키워드 10개를 추천해줘.

★ 반드시 이 형태여야 함:
- 브랜드+카테고리 (예: 스타벅스 봄메뉴, 나이키 신상, 다이소 추천템)
- 계절/트렌드 키워드 (예: 봄신상, 체리블라썸 음료, 봄 코디)
- 카테고리 수준 (예: 편의점 신상과자, 봄 가디건, 봄 캠핑용품)

★ 절대 안 되는 것 (이런 게 나오면 0점):
- 사람 이름 포함 (강소라, 곽튜브, 닥터신, 고현정 등 어떤 인물이든)
- 드라마/영화 제목 (닥터신, 세이렌, 콘크리트 마켓 등)
- 범용어 단독 (리뷰, 비법, 이야기, 출연진)
- 특정 모델명/시리얼
- 급상승 단어를 그대로 쓰는 것
...
async function collectBlogTitles(netKeywords) {
  ...
  const mimeCandidates = Object.entries(ngramFreq)
    .filter(([gram, data]) => {
      ...
      if (gram.length < 3) return false; // 3자 미만 단어 제외
      if (data.recent < 8) return false; // 최소 8회 이상 등장
      if (data.categories.size < 4) return false; // 4개 이상 카테고리
      const older = data.older || 0;
      // 이전 3일 대비 4배 이상 급등 (또는 완전 신규)
      if (older === 0) return data.recent >= 8;
      return (data.recent / older) >= 4;
    })
    .sort((a, b) => {
      // 교차 카테고리 수 × 급등률로 정렬
...
찾으면 안 되는 것:
- 일반 한국어 단어 (레시피, 후기, 일상, 추천, 캠핑, 골드)
- 브랜드명, 제품명 (쿨더마, 갈바닉, 닥터신 등 제품/브랜드)
- 연예인/인물 이름
- 뉴스성 키워드 (사고, 날씨, 정치)
- 단어 일부 조각 (2~3글자 단독, 예: 뮤직, 골드, 쿨더)
- 오래된 유행어 (갓생, 무지출 등 이미 일반화된 표현)

중요: 진짜 밈/신조어는 의미가 불명확하고 재미있는 맥락으로 쓰이는 표현이야.
제품 홍보나 연예인 기사에 쓰이는 단어는 밈이 아니야.
